...
//	stepperDriver.move(A4954_NUM_MICROSTEPS,NVM->SystemParams.currentMa); //move one full step 'forward' /Just to compile
...
int StepperCtrl::begin(void)
{
	...
	//stepperDriver.begin(); //JUST TO COMPILE
...
//		n=A4954_NUM_MICROSTEPS*fullStepsPerRotation;
		...
		//stepperDriver.move(n,NVM->SystemParams.currentMa); //JUST TO COMPILE
	}
	currentLimit=false;
...
bool StepperCtrl::process(void)
{
	...
	{  //feedback system is not enabled
		//if (stepperDriver.microsSinceStep()>200) ///JUST TO COMPILE
		{
